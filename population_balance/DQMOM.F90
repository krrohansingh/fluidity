...
contains
  ...
  subroutine dqmom_calculate_source_term_pop(state, it, i_pop, states, cont_state)
    ...
    if (have_option(trim(option_path)//'/population_balance_source_terms/aggregation')) then
       ...
       else if (have_option(trim(option_path)//'/population_balance_source_terms/aggregation/aggregation_frequency/prince_blanch_aggregation')) then
          aggregation_freq_type = 'prince_blanch_aggregation'
          if (.not. have_option("/population_balance_continuous_phase_name")) then
             FLAbort("Enable the option population_balance_continuous_phase_name and provide a name for the continuous phase&
                      as it is needed for extracting the turbulence dissipation needed in prince_blanch_aggregation kernel")
          end if
          turbulent_dissipation => extract_scalar_field(states(cont_state), "TurbulentDissipation", stat=stat)
            if (stat/=0) then
             FLAbort("I can't find the Turbulent Dissipation field of continuous phase for population balance aggregation term calculations.")
          end if
       else if (have_option(trim(option_path)//'/population_balance_source_terms/aggregation/aggregation_frequency/luo_aggregation')) then
          aggregation_freq_type = 'luo_aggregation'
          if (.not. have_option("/population_balance_continuous_phase_name")) then
             FLAbort("Enable the option population_balance_continuous_phase_name and provide a name for the continuous phase&
                      as it is needed for extracting the turbulence dissipation needed in prince_blanch_aggregation kernel")
          end if
    ...
    if (have_option(trim(option_path)//'/population_balance_source_terms/breakage')) then
       ...
       if (.not. have_option("/population_balance_continuous_phase_name")) then
        ...
        else if (have_option(trim(option_path)//'/population_balance_source_terms/breakage/breakage_frequency/prince_blanch_breakage')) then
           breakage_freq_type = 'prince_blanch_breakage'
           if (.not. have_option("/population_balance_continuous_phase_name")) then
                FLAbort("Enable the option population_balance_continuous_phase_name and provide a name for the continuous phase &
                         as it is needed for extracting the turbulence dissipation needed in laakkonen_frequency kernel")
           end if
       ...
       else if (have_option(trim(option_path)//'/population_balance_source_terms/breakage/distribution_function/coulaloglou_tavlarides')) then
          breakage_dist_type = 'coulaloglou_tavlarides'
       end if
  ...
  subroutine dqmom_calculate_source_term_node(abscissa, weight, s_weighted_abscissa, s_weight, &
    ...
    integer :: stat, N, i, j, k, nin
    double precision f, r, s, integral
    external f,g

    real :: sigma, density_continuous, density_dispersed, weber
    ...
    if (have_breakage) then
	  ...
	  eps_node = node_val(turbulent_dissipation,node)
	  do i = 1, N
	     break_freq(:,i) = K1*(1./abscissa_val(1,i)**(2./3))*eps_node**(1./3) *exp(-K2*sigma*(1./(density_dispersed*eps_node**(2./3) *abscissa_val(1,i)**(5./3))))
	  end do
       else if (breakage_freq_type=='prince_blanch_breakage') then
           
           eps_node = node_val(turbulent_dissipation,node)
             do i = 1, N
             r = 0.2 * abscissa_val(1,i)
             s = abscissa_val(1,i)
             nin = 2
             do k=1,16
                call simpson(r,s,integral,nin, breakage_freq_type,eps_node )
                nin = nin*2
             end do
             break_freq(:,i) = integral
          end do
       end if
       ...
       else if (breakage_dist_type=='coulaloglou_tavlarides') then
          do i = 1, 2*N
             do j = 1, N
                r = 0
                s = abscissa_val(1,j)
                nin = 2
                do k=1,16
                   call simpson2(r,s,integral,nin, breakage_dist_type,eps_node, i)
                   nin = nin*2
                end do
                moment_daughter_dist_func(1,i,j) = integral
             end do
          end do
       end if
    ...
    if (have_aggregation) then
       if (aggregation_freq_type=='constant_aggregation') then
         ...
         allocate(visc_node(viscosity_continuous%dim(1), viscosity_continuous%dim(1)))
          ...
          do i = 1, N
             do j = 1, N
          aggregation_freq(1,i,j) = K3 * eps_node**(1./3) * (abscissa_val(1,i)**2 + abscissa_val(1,j)**2) * (abscissa_val(1,i)**(2./3) + abscissa_val(1,j)**(2./3))**(1./2) * exp(-K4*((visc_node(1,1)*density_continuous)/sigma**2)*eps_node*((abscissa_val(1,i)*abscissa_val(1,j))/(abscissa_val(1,i)+abscissa_val(1,j)))**4)
              end do   
          end do
	  deallocate(visc_node)
       else if (aggregation_freq_type=='prince_blanch_aggregation') then
          density_continuous = 998.2
          sigma = 0.072
          eps_node = node_val(turbulent_dissipation,node)
          do i = 1, N
             do j = 1, N
          aggregation_freq(1,i,j) = 0.2797 * eps_node**(1./3) * (abscissa_val(1,i) + abscissa_val(1,j))**2 * (abscissa_val(1,i)**(2./3) + abscissa_val(1,j)**(2./3))**(1./2) * exp(-0.54395*(eps_node**(1./3) * (density_continuous/sigma)**(1./2))*((abscissa_val(1,i)*abscissa_val(1,j))/(abscissa_val(1,i)+abscissa_val(1,j)))**(5./6))
              end do   
          end do
       else if (aggregation_freq_type=='luo_aggregation') then
          density_continuous = 998.2
          density_dispersed = 1.205
          sigma = 0.072
          eps_node = node_val(turbulent_dissipation,node)
          do i = 1, N
             do j = 1, N
          weber= density_continuous* abscissa_val(1,i)* 1.4 *eps_node**(1./3) * (abscissa_val(1,i)**(2./3) + abscissa_val(1,j)**(2./3))**(1./2) * (1./sigma)
          aggregation_freq(1,i,j) = 4.4447 * eps_node**(1./3) * (abscissa_val(1,i) + abscissa_val(1,j))**2 * (abscissa_val(1,i)**(2./3) + abscissa_val(1,j)**(2./3))**(1./2) * exp(- (0.75*(1 + (abscissa_val(1,i)/abscissa_val(1,j))**2) *(1 + (abscissa_val(1,i)/abscissa_val(1,j))**3))**(1./2) *weber**(1./2) * 1./((density_dispersed/density_continuous + 0.5) *(1 + abscissa_val(1,i)/abscissa_val(1,j))**3))
             end do
          end do 
       end if
       do i = 1, 2*N
...
function f(x, s, breakage_freq_type, eps_node)
   real :: density_continuous = 998.2
   real :: sigma = 0.072
   double precision f, x, s, eps_node
   integer i
   character(len=FIELD_NAME_LEN), intent(in) :: breakage_freq_type
   if (breakage_freq_type=='prince_blanch_breakage') then
      f = ((0.07 * 3.1415926**4)/x**4) *(s + x)**2 * (s**(2./3) + x**(2./3))**(1./2) * eps_node**(1./3) * exp (-1.18*sigma/(x**(2./3) *density_continuous*s*eps_node**(2./3)))
   end if
   end function f

function g(x, s, breakage_dist_type, eps_node, i)   
   double precision g, x, s, eps_node
   integer i
   character(len=FIELD_NAME_LEN), intent(in) :: breakage_dist_type
   if (breakage_dist_type=='coulaloglou_tavlarides') then
      g = 7.2*x**(2+i-1)*(1./s**3) * exp(-4.5*(2*x**3 - s**3)**2 *(1./s**6))
   end if
   end function g

  Subroutine simpson(r,s,integral,nin, breakage_freq_type, eps_node)
! Integration of f(x) on [r,s]
  implicit none
  double precision  r, s,integral, t
  double precision h, x, eps_node
  integer nin, i
  character(len=FIELD_NAME_LEN), intent(in) :: breakage_freq_type
! if nin is odd we add +1 to make it even
  if((nin/2)*2.ne.nin) nin=nin+1

! loop over nin (number of intervals)
  t = 0.0
  h = (s-r)/dfloat(nin)
  do i=2, nin-2, 2
   x   = r+dfloat(i)*h
   t = t + 2.0*f(x,s, breakage_freq_type, eps_node) + 4.0*f(x+h,s, breakage_freq_type, eps_node)
  end do
  integral = (t + f(r, s,breakage_freq_type, eps_node) + f(s,s,breakage_freq_type, eps_node) + 4.0*f(r+h,s,breakage_freq_type, eps_node))*h/3.0
end subroutine simpson

 Subroutine simpson2(r,s,integral,nin, breakage_dist_type, eps_node, i)
! Integration of g(x) on [r,s]
  implicit none
  double precision  r, s,integral, t
  double precision h, x, eps_node
  integer nin, i, j
...
! loop over nin (number of intervals)
  ...
  do j=2, nin-2, 2
   x   = r+dfloat(i)*h
   t = t + 2.0*g(x,s, breakage_dist_type, eps_node, i) + 4.0*g(x+h,s, breakage_dist_type, eps_node, i)
  end do
  integral = (t + g(r, s,breakage_dist_type, eps_node, i) + g(s,s,breakage_dist_type, eps_node, i) + 4.0*g(r+h,s,breakage_dist_type, eps_node, i))*h/3.0
